...
class Forgetpass_S extends StatefulWidget {
  const Forgetpass_S({super.key});

  @override
  State<Forgetpass_S> createState() => _ForgetState();
}

class _ForgetState extends State<Forgetpass_S> {
  ...
  Future<void> checkUserData(String username) async {
    final url = Uri.parse('https://www.edueliteroom.com/connect/forgetpassStudents.php');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'success') {
          userStudents = UserStudents.fromJson(data['data']);  // เก็บข้อมูล UserTeacher
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('ข้อมูลผู้ใช้ ${userStudents?.usersUsername} ถูกต้อง กรุณากรอกอีเมล'),
  ...
  Future<void> sendDataToApi(String username, String password) async {
    final url = Uri.parse('https://www.edueliteroom.com/connect/update_passwordStudents.php');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        context,
        MaterialPageRoute(builder: (context) => Login_S()),  // เปลี่ยน Login_T เป็นหน้าที่ต้องการ
      );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> checkEmail(String email) async {
    ...
    if (userStudents?.usersEmail == userEmail) {
  ...
  Future<void> checkPhone(String phone) async {
    ...
    if (userStudents?.usersPhone == userPhone) {
