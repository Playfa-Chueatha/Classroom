    ...
    try {
        ...
        if ($user) {
            ...
            if ($classroom) {
                ...
                if ($studentInClass) {
                    // หากมีข้อมูลใน students_inclass ให้ทำการอัปเดต inclass_type เป็น 0
                    $stmt = $conn->prepare("UPDATE students_inclass SET inclass_type = 0 WHERE users_username = :users_username AND classroom_id = :classroom_id");
                    $stmt->bindParam(':users_username', $users_username);
                    $stmt->bindParam(':classroom_id', $classroom_id);
                    $stmt->execute();
