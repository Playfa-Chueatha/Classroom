...
try {
    ...
    $stmt1 = $conn->prepare("
        SELECT 
            notification_sibmit_auto, 
            notification_sibmit_title, 
            notification_sibmit_classID, 
            notification_sibmit_time, 
            notification_sibmit_usert, 
            notification_sibmit_readstatus,
            notification_sibmit_users
        FROM notification_sibmit
        WHERE notification_sibmit_usert = :username
    ");
    ...
    $classroomIds = array_column($notifications, 'notification_sibmit_classID');
    ...
    foreach ($notifications as $key => $notification) {
        foreach ($classroomData as $classroom) {
            if ($notification['notification_sibmit_classID'] == $classroom['classroom_id']) {
